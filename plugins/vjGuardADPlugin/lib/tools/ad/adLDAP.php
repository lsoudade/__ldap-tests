...
class adLDAP {
    ...
    */
     ...
     */
          /*
    // Pour pallier au "Insufficient access"
    $this->_bind = ldap_bind($this->_conn,"CN=lionel,OU=people,DC=example,DC=com",$this->_ad_password);            
          
    // Recherche par nom
    $sr=ldap_search($this->_conn,"OU=people,DC=example,DC=com", "CN=*");  
    echo 'Le résultat de la recherche est ' . $sr . '<br />';

    echo 'Le nombre d\'entrées retournées est ' . ldap_count_entries($this->_conn,$sr) 
         . '<br />';
